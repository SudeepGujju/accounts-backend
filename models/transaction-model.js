...
const { ACCOUNTS_COLLECTION_NAME, USERS_COLLECTION_NAME, TRANSACTIONS_COLLECTION_NAME } = require("../constants");
...
// transactionSchema.index( {userId: 1, code: 1}, {unique: true});

const transactionJoiSchema = (minDate, maxDate) => {
    return Joi.object({
...
function validateTransactionData(data, minDate, maxDate) {

    const schema = transactionJoiSchema(minDate, maxDate);

    return schema.validate(data, { stripUnknown: true });
}

const TransactionModel = model(TRANSACTIONS_COLLECTION_NAME, transactionSchema);
